...
(FTS5 + Qdrant) и wiki-слоя по Карпатыху. Чат — корпоративная Qwen через
LiteLLM; эмбеддинги — отдельной моделью со своим endpoint'ом (или тем же
LiteLLM, если embed_base_url пуст). Развёртывание — в закрытом контуре
(bundle + deploy.sh), разработка — здесь, на mock LLM.

## Команды

```bash
.venv/bin/python -m pytest tests -q        # тесты (59 шт., mock LLM)
...
- Эмбеддинги — отдельной моделью через собственный OpenAI-совместимый
  endpoint (`embed_base_url`); если он пуст — через основной LiteLLM.
...
- Смена модели эмбеддингов (другая размерность): VectorStore при первой
  записи сам пересоздаёт коллекцию, но проекту нужна полная переиндексация;
  до неё поиск по векторам честно возвращает пусто (см. warning в журнале).
  question_log хранит старые эмбеддинги вопросов — FAQ-кластеризация
  деградирует, пока лог не перезатрётся новыми вопросами.
